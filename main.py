...
def run(args):
    ...
    for i in range(len(task_names)):
        ...
        # get correlation
        # if train_name == '1':
        #     grad = agent.train_task_correlation(train_loader,val_loader)
        #     #print('grad:',grad[0].shape)
        #     lambd = 0.01
        # else:
        #     grad_pre = copy.deepcopy(grad)
        #     grad = agent.train_task_correlation(train_loader,val_loader)
        ...
        #         projection = np.dot(grad[i],grad_pre[i].T)
        #         projection_norm = np.linalg.norm(projection)
        #         if projection_norm > 0.7 * grad_norm:
        #             lambd = 0.01
        #         else:
        #             lambd = 0.02
        # Learn
        ...
        print('Time for training one task:  ',time_train)
